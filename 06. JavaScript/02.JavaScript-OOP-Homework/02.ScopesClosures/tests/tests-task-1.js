...
describe('Tests for Closures and Scopes in JavaScript', function () {
	...
	describe('"Task 1: Library"', function () {
		...
		it('expect 1 library to exist', function () {
			expect(library).to.exist;
		});

		it('expect 2 library.books to exist', function () {
			expect(library.books).to.exist;
		});

		it('expect 3 library.categories to exist', function () {
			expect(library.categories).to.exist;
		});

		describe('library.books.add', function () {
			it('expect 4 to exist and to be a function', function () {
				expect(library.books.add).to.exist;
				expect(library.books.add).to.be.a('function');
			});

			it('expect 5 the book to be returned with generated ID(number greater than 1), when title, ISBN and author are valid', function () {
				var bookToAdd = {
			...
			it('expect 6 to throw, when missing title, category, author and isbn', function () {
				function test() {
					library.books.add();
				}
			...
			it('expect 7 to throw, when with invalid title (short)', function () {
				function test() {
					library.books.add({
			...
			it('expect 8 to throw, when with invalid title (long)', function () {
				function test() {
					library.books.add({
			...
			it('expect 9 to throw, when with invalid isbn', function () {
				function test() {
					library.books.add({
			...
			it('expect 10 to throw, when with invalid author', function () {
				function test() {
					library.books.add({
						title: CONSTS.VALID.AUTHOR,
						isbn: CONSTS.VALID.BOOK_ISBN.TEN_DIGITS,
			...
			it('expect 11 to throw, when book with repeating title is added', function () {
				var book = {
					title: CONSTS.VALID.BOOK_TITLE,
					isbn: CONSTS.VALID.BOOK_ISBN.TEN_DIGITS,
					author: CONSTS.VALID.AUTHOR,
			...
			it('expect 12 to throw, when book with repeating ISBN is added', function () {
				var book = {
					title: CONSTS.VALID.BOOK_TITLE,
					isbn: CONSTS.VALID.BOOK_ISBN.TEN_DIGITS,
					author: CONSTS.VALID.AUTHOR,
		...
		describe('library.books.list', function () {
			it('expect 13 to exist and to be a function', function () {
				expect(library.books.list).to.exist;
				expect(library.books.list).to.be.a('function');
			});
			it('expect 14 to return empty array, when no books are added', function () {
				expect(library.books.list()).to.eql([]);
			});
			it('expect 15 to return array with single book, when a single book is added', function () {
			...
			it('expect 16 to return empty array, when category is provided and no book in that category is added', function () {
					var book = library.books.add({
						title: CONSTS.VALID.BOOK_TITLE,
			...
			it('expect 17 to return array with single book, ' +
			...
			it('expect 18 to return array with single book, ' +
			...
			it('expect 19 to return empty array, ' +
			...
			it('expect 20 to return empty array, ' +
		...
		describe('library.categories.list', function () {
			it('expect 21 to exist and to be a function', function () {
				expect(library.categories.list).to.exist;
				expect(library.categories.list).to.be.a('function');
			});

			it('expect 22 to return empty array, when no books are added', function () {
				expect(library.categories.list()).to.eql([]);
			});

			it('expect 23 to return array with single category, when books from the same category are added', function () {
				var book = {
			...
			it('expect 24 to return array with single category, when a single book is added', function () {
				var book = {
					title: CONSTS.VALID.BOOK_TITLE + 1,
					isbn: CONSTS.VALID.BOOK_ISBN.TEN_DIGITS,
					author: CONSTS.VALID.AUTHOR,
			...
			it('expect 25 to return array with correctly ordered categories, when there are books of a different category', function () {
				var books = [{
